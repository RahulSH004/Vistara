...
export async function createbooking(data, user_id) {
    ...
    const overlapquery = await db.select()
        .from(bookings)
        .where(and(eq(bookings.room_id, roomId), lt(bookings.check_in_date, checkOutDateStr), gt(bookings.check_out_date, checkInDateStr), ne(bookings.status, "cancelled")));
